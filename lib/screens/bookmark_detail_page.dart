...
class _BookmarkDetailPageState extends State<BookmarkDetailPage> {
  ...
  bool _isBookmarked =
      true; // by definition this page is opened from a bookmark
  int? _bookmarkId;
  ...
  void initState() {
    super.initState();
    _bookmarkId = widget.entry.bookmarkId;
    _loadHadithDetail();
  }
  ...
  Future<void> _loadHadithDetail() async {
    ...
    try {
      final result = await ApiService.getHadithDetailWithBookmark(
        hadithId: widget.entry.hadithId,
  ...
  Future<void> _toggleBookmark() async {
    if (!_isBookmarked) {
      // If trying to bookmark again, this shouldn't happen from bookmark page
      // But handle it gracefully by creating a new bookmark
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Failed to add bookmark: ${e.toString()}'),
              backgroundColor: Colors.red,
    ...
    try {
      if (_bookmarkId != null) {
        ...
        await ApiService.deleteBookmark(bookmarkId: widget.entry.bookmarkId);
      }
